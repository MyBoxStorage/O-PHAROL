    ...
    >
      ...
      <div className="container" style={{ position: 'relative', zIndex: 1 }}>
        ...
        <div className="footer-grid" style={{ display: 'grid', gridTemplateColumns: '1.4fr 1fr 1fr 1fr', gap: 32 }}>
          ...
          {/* Navegação */}
          <div>
            <div style={{ color: 'var(--gold)', marginBottom: 18, textTransform: 'uppercase', letterSpacing: '0.22em', fontSize: '0.56rem', fontWeight: 700 }}>{t.footer.navTitle}</div>
            <div style={{ display: 'grid', gap: 2 }}>
              {[
                { label: t.footer.links.restaurant, href: '#about' },
                { label: t.footer.links.menu, href: '#menu' },
                { label: t.footer.links.history, href: '#history' },
                { label: t.footer.links.location, href: '#location' },
              ].map(({ label, href }) => (
              ...
              <button onClick={onReserve} style={linkStyle}
                onMouseEnter={(e) => (e.currentTarget.style.color = 'var(--gold)')}
                onMouseLeave={(e) => (e.currentTarget.style.color = 'rgba(255,255,255,0.55)')}
              >{t.footer.links.reservations}</button>
              <button onClick={onQueue} style={linkStyle}
                onMouseEnter={(e) => (e.currentTarget.style.color = 'var(--gold)')}
                onMouseLeave={(e) => (e.currentTarget.style.color = 'rgba(255,255,255,0.55)')}
              >{t.footer.links.virtualQueue}</button>
            </div>
          </div>

          {/* Área do Cliente */}
          <div>
            <div style={{ color: 'var(--gold)', marginBottom: 18, textTransform: 'uppercase', letterSpacing: '0.22em', fontSize: '0.56rem', fontWeight: 700 }}>{t.footer.clientTitle}</div>
            <div style={{ display: 'grid', gap: 2 }}>
              {[
                { label: t.footer.clientLinks.myArea, fn: onClientArea },
                { label: t.footer.clientLinks.myReservations, fn: onClientArea },
                { label: t.footer.clientLinks.loyalty, fn: onClientArea },
                { label: t.footer.clientLinks.visitHistory, fn: onClientArea },
              ].map(({ label, fn }) => (
          ...
          {/* Contato */}
          <div>
            <div style={{ color: 'var(--gold)', marginBottom: 18, textTransform: 'uppercase', letterSpacing: '0.22em', fontSize: '0.56rem', fontWeight: 700 }}>{t.footer.contactTitle}</div>
        ...
        <div className="footer-bottom" style={{ marginTop: 56, paddingTop: 20, borderTop: '1px solid rgba(255,255,255,0.06)', display: 'flex', gap: 16, justifyContent: 'space-between', flexWrap: 'wrap', fontSize: '0.7rem', color: 'rgba(255,255,255,0.3)' }}>
          ...
          <a href="https://globallanding.com.br" target="_blank" rel="noreferrer" style={{ color: 'rgba(255,255,255,0.3)', transition: 'color 0.3s' }}
            ...
            onMouseLeave={(e) => (e.currentTarget.style.color = 'rgba(255,255,255,0.3)')}
          >
            {t.footer.devBy} <span style={{ color: 'var(--gold)' }}>Global Landing</span>
          </a>
        </div>
      </div>
    </footer>
